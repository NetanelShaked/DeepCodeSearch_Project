...
def run():
    ...
    for epoch in range(config.Epochs):
        train_accuracy, train_loss = engine.train_fn(teacher_model, train_loader, teacher_optimizer, teacher_scheduler,
                                                     device)
        ...
        print(
            f'EPOCH -> {epoch + 1}/{config.Epochs} | TEACHERS TRAIN LOSS = {train_loss} | TEACHERS TRAIN ACC = {train_accuracy * 100}% | TEACHERS VAL LOSS = {val_loss} | TEACHER VAL ACC = {val_accuracy * 100}%\n')
    ...
    for epoch in range(config.Epochs):
        train_accuracy, train_loss = engine.train_fn(student_model, train_loader, student_optimizer, student_scheduler,
                                                     device)
        ...
        print(
            f'EPOCH -> {epoch + 1}/{config.Epochs} | STUDENTS TRAIN LOSS = {train_loss} | STUDENTS TRAIN ACC = {train_accuracy * 100}% | STUDENTS VAL LOSS = {val_loss} | STUDENTS VAL ACC = {val_accuracy * 100}%\n')
    ...
    for epoch in range(config.Epochs):
        ...
        print(
            f'EPOCH -> {epoch + 1}/{config.Epochs} | TEACHER STUDENT TRAIN LOSS = {train_loss}  | TEACHER STUDENT TRAIN ACC = {train_accuracy * 100}% | TEACHER STUDENT VAL LOSS = {val_loss} | TEACHER STUDENT VAL ACC = {val_accuracy * 100}%\n')
        if best_acc < val_accuracy:
            best_acc = val_accuracy
            teacher_student_best_dict = teacher_student_model.student.state_dict()
